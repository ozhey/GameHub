...
public interface TTTScoreRepository extends JpaRepository<TTTScore, Long> {
    ...
    @Query("SELECT u.username, COUNT(t) AS gamesPlayed, SUM(CASE WHEN t.didWinGame THEN 1 ELSE 0 END) AS gamesWon FROM TTTScore t JOIN t.user u GROUP BY u.username ORDER BY gamesWon DESC")
